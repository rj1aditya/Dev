...
int LinkList::searchPosition( int val )
{
	Data *temp = head;
...
void LinkList::insertNthpos(int pos, Data* newData )
{	
	Data *prev;
	Data *temp = head;
...
void LinkList::InsertElement()
{
	...
	{
		int pos = searchPosition( newData->LunNo );
		insertNthpos( pos, newData );
	}
}
...
// below logic if to find loop using set container.
...
{
	//e.g: 1 3 2 8 1 0 3->null
	//o/p: 1 3 2 8 0
	Data *prev = NULL;
	
	for( Data *outer = head; outer->next->next != NULL; outer = outer->next )
	{
		for( Data *inner = outer->next; inner != NULL; )
		{
			if( outer->LunNo == inner->LunNo )
			{
